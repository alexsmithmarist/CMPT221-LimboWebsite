...
<html>
  ...
  <body>
    ...
    <p class="description"> Here is the database of lost items: </p>
    <?php 
      ...
      //Gets the lost items from the database.
      $query = 'SELECT create_date, item_status, item_name FROM stuff WHERE item_status = "lost" ORDER BY create_date DESC' ;
      ...
      if($_SERVER['REQUEST_METHOD'] == 'POST'){
          ...
          if(empty($search))
          {
              ...
              {
                  $query = 'SELECT create_date, item_status, item_name FROM stuff WHERE location_id = ' . $loc . ' AND item_status = "lost" ORDER BY create_date DESC' ;
              }
          }
          ...
          else
          {
              if($loc == 35)
              {
                  $query = 'SELECT create_date, item_status, item_name FROM stuff WHERE item_name LIKE "%' . trim($search) . '%" AND item_status = "lost" ORDER BY create_date DESC' ;
              }
              else
              {
                  $query = 'SELECT create_date, item_status, item_name FROM stuff WHERE item_name LIKE "%' . trim($search) . '%" AND location_id = ' . $loc . ' AND item_status = "lost" ORDER BY create_date DESC' ;
              }
          }
      }
	  ...
	  if( $results )
	  {
        ...
        echo '</TR>';
  		...
  		{
            $alink = '<A HREF=lost.php?item_name=' . $row['item_name']  . '>' . $row['item_name'] . '</A>' ;
    		echo '<TR>' ;
    ...
    ?>
      <a href = "lostform.php" class="submitnew"> Submit a new Lost item</a>
