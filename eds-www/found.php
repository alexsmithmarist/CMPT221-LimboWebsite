...
<html>
  ...
  <header>
    <title> Found Item Inventory </title>
  ...
  <body>
    ...
    <h1> Found Item Inventory </h1>


    <!--Search boxes for the table -->
    <form action="found.php" method="POST" name='foundsearch'>
    ...
    <br>
      ...
      <p class="description"> Here is the database of found items: </p>
        <?php 
          //Connect to the database and retrieve the helping php functions from separate files
          require('includes/connect_db.php');
          require('includes/helpers.php');
      
          //Gets the found items from the database.
          $query = 'SELECT create_date, item_status, item_name FROM stuff WHERE item_status = "found" ORDER BY create_date DESC' ;
          ...
          if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if(empty($search))
            {
              ...
              {
                  $query = 'SELECT create_date, item_status, item_name FROM stuff WHERE location_id = ' . $loc . ' AND item_status = "found" ORDER BY create_date DESC' ;
              }
            }
            ...
            else
            {
              if($loc == 35)
              {
                $query = 'SELECT create_date, item_status, item_name FROM stuff WHERE item_name LIKE "%' . trim($search) . '%" AND item_status = "found" ORDER BY create_date DESC' ;
              }
              else
              {
                  $query = 'SELECT create_date, item_status, item_name FROM stuff WHERE item_name LIKE "%' . trim($search) . '%" AND location_id = ' . $loc . ' AND item_status = "found" ORDER BY create_date DESC' ;
              }
            }
	...
	if( $results )
	{
        ...
        echo '</TR>';
  		...
  		{
            $alink = '<A HREF=found.php?item_name=' . $row['item_name']  . '>' . $row['item_name'] . '</A>' ;
    		echo '<TR>' ;
    ...
    ?>
    <a class="submitnew" href = "foundform.php"> Submit a new Found item</a>
