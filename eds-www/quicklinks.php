...
<html>
  ...
  <body>
    ...
    <h1>Items in Limbo</h1>
      ...
      <form action="quicklinks.php" method="POST">
        <p class="description">Search by keyword:
            <input type='text' name='searchkey'
           value ='<?php if (isset( $_POST['searchkey']))
            echo $_POST['searchkey']; ?>'> </p>
      ...
      <option value=35 selected>Any Location</option>
          ...
          <p><input class="update" type="submit" value="Update Search" ></p>
      ...
      //Gets the non-claimed items from the database.
          $query = 'SELECT stuff.create_date, locations.name, stuff.item_status, stuff.item_name
          FROM stuff, locations
          WHERE item_status <> "claimed" AND locations.id = stuff.location_id' ;
      ...
      if($_SERVER['REQUEST_METHOD'] == 'POST'){
          ...
          if(empty($search))
          {
              ...
              if($loc == 35)
              {
                  $query = 'SELECT stuff.create_date, locations.name, stuff.item_status, stuff.item_name FROM stuff, locations WHERE stuff.item_status <> "claimed" AND stuff.location_id = locations.id ORDER BY stuff.create_date DESC' ;
              }
              else
              {
                  $query = 'SELECT stuff.create_date, locations.name, stuff.item_status, stuff.item_name FROM stuff, locations WHERE location_id = ' . $loc . ' AND item_status <> "claimed" AND stuff.location_id = locations.id ORDER BY create_date DESC' ;
              }
          }
          //If the search has an input, search for an item with input in the name
          else{
              if($loc == 35){
              $query = 'SELECT stuff.create_date, locations.name, stuff.item_status, stuff.item_name FROM stuff, locations WHERE item_name LIKE "%' . trim($search) . '%" AND item_status <> "claimed" AND stuff.location_id = locations.id ORDER BY create_date DESC' ;
              }
              else{
                  $query = 'SELECT stuff.create_date, locations.name, stuff.item_status, stuff.item_name FROM stuff, locations WHERE item_name LIKE "%' . trim($search) . '%" AND location_id = ' . $loc . ' AND item_status <> "claimed" AND stuff.location_id = locations.id ORDER BY create_date DESC' ;
              }
        }
      }
	...
	if( $results )
	{
        ...
        echo '</TR>';
  		...
  		{
            $alink = '<A HREF=quicklinks.php?item_name=' . $row['item_name']  . '>' . $row['item_name'] . '</A>' ;
    		echo '<TR>' ;
